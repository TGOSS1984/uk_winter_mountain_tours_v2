...
class Booking(models.Model):
    ...
    class Meta:
        constraints=[models.UniqueConstraint(fields=['guide','date','time_slot'], name='unique_guide_timeslot')]
    ...
    def clean(self):
        ...
        if self.guide_id and self.date and self.time_slot:
            conflict = (
                Booking.objects
                .exclude(pk=self.pk)
                .filter(guide_id=self.guide_id, date=self.date, time_slot=self.time_slot, status='confirmed')
                .exists()
            )
            if conflict:
