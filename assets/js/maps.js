...
// Prefix GPX paths with Django static (expects window.STATIC_PREFIX set in the template)
const STATIC_PREFIX = (window.STATIC_PREFIX || '/static/').replace(/\/+$/, '/');
const toStatic = (p) => (p.startsWith('http') ? p : STATIC_PREFIX + p.replace(/^\/+/, ''));
...
function loadMap(mapId, gpxPath, lat, lon, zoom = 13) {
  ...
  new L.GPX(toStatic(gpxPath), {
    ...
    .on('error', () => {
      console.warn(`Failed to load GPX for ${mapId}: ${gpxPath}`);
    })
    .addTo(map);
}

// Lake District routes (now under assets/routes/… → /static/routes/…)
